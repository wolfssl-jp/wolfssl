...
#if !defined(NO_FILESYSTEM) && !defined(NO_CERTS) && !defined(NO_WOLFSSL_SERVER)
    WOLFSSL_CTX *ctx;
    printf(testingFmt, "wolfSSL_CTX_use_PrivateKey_file(()");
    AssertNotNull(ctx = wolfSSL_CTX_new(wolfSSLv23_server_method()));
...
static void test_wolfSSL_CTX_load_verify_locations(void)
{
    ...
    WOLFSSL_CERT_MANAGER *cm;
        ...
        int cacheSz;
#endif
    printf(testingFmt, "test_wolfSSL_CTX_load_verify_locations(()");
    AssertNotNull(ctx = wolfSSL_CTX_new(wolfSSLv23_client_method()));
...
void ApiTest(void)
{
    ...
    AssertIntEQ(test_EccSigFailure_cm(), ASN_NO_SIGNER_E);
